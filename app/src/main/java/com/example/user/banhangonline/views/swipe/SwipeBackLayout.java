...
public class SwipeBackLayout extends ViewGroup {
    ...
    private SwipeBackLayout.DragEdge drgEdge = SwipeBackLayout.DragEdge.NULL;

    public void setDrgEdge(SwipeBackLayout.DragEdge drgEdge) {
        this.drgEdge = drgEdge;
    }
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs) {
        ...
        viewDragHelper = ViewDragHelper.create(this, 1.0f, new SwipeBackLayout.ViewDragHelperCallBack());
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (drgEdge == DragEdge.NULL) return;
        switch (drgEdge) {
            case TOP:
            case BOTTOM:
    ...
    private int getDragRange() {
        if (drgEdge == DragEdge.NULL) return verticalDragRange;
        switch (drgEdge) {
            case TOP:
            case BOTTOM:
    ...
    private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            if (drgEdge == DragEdge.NULL) return 0;

            int result = 0;
            if (drgEdge == SwipeBackLayout.DragEdge.TOP && !canChildScrollUp() && top > 0) {
                final int topBound = getPaddingTop();
                final int bottomBound = verticalDragRange;
                result = Math.min(Math.max(top, topBound), bottomBound);
            } else if (drgEdge == SwipeBackLayout.DragEdge.BOTTOM && !canChildScrollDown() && top < 0) {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            int result = 0;
            if (drgEdge == SwipeBackLayout.DragEdge.LEFT && !canChildScrollRight() && left > 0) {
                final int leftBound = getPaddingLeft();
                final int rightBound = horizontalDragRange;
                result = Math.min(Math.max(left, leftBound), rightBound);
            } else if (drgEdge == SwipeBackLayout.DragEdge.RIGHT && !canChildScrollLeft() && left < 0) {
    ...
    private boolean backBySpeed(float xvel, float yvel) {
        if (drgEdge == DragEdge.NULL) return false;

        switch (drgEdge) {
            case TOP:
            case BOTTOM:
                if (Math.abs(yvel) > Math.abs(xvel) && Math.abs(yvel) > AUTO_FINISHED_SPEED_LIMIT) {
                    return drgEdge == SwipeBackLayout.DragEdge.TOP ? !canChildScrollUp() : !canChildScrollDown();
                }
                break;
            case LEFT:
            case RIGHT:
                if (Math.abs(xvel) > Math.abs(yvel) && Math.abs(xvel) > AUTO_FINISHED_SPEED_LIMIT) {
                    return drgEdge == SwipeBackLayout.DragEdge.LEFT ? !canChildScrollLeft() : !canChildScrollRight();
                }
                break;
        }
        return false;
